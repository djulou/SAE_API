...
from sqlalchemy import (
    Integer, BigInteger, String, Text, Float, Date, DateTime, 
...
class License(Base):
    ...
    license_name: Mapped[Optional[str]] = mapped_column(String(150))

class Tag(Base):
    __tablename__ = 'tag'
    tag_id: Mapped[int] = mapped_column(primary_key=True)
    tag_name: Mapped[Optional[str]] = mapped_column(String(100))

class Genre(Base):
    __tablename__ = 'genre'
    genre_id: Mapped[int] = mapped_column(primary_key=True)
    genre_parent_id: Mapped[Optional[int]] = mapped_column(ForeignKey('sae.genre.genre_id', ondelete="CASCADE"))
    genre_title: Mapped[Optional[str]] = mapped_column(String(50))
    genre_handle: Mapped[Optional[str]] = mapped_column(String(50))
...
class Artist(Base):
    ...
    artist_id: Mapped[int] = mapped_column(BigInteger, primary_key=True, autoincrement=False)
    artist_handle: Mapped[Optional[str]] = mapped_column(String(150))
    ...
    artist_members: Mapped[Optional[str]] = mapped_column(String(7000))
    artist_associated_labels: Mapped[Optional[str]] = mapped_column(String(255))
    artist_related_projects: Mapped[Optional[str]] = mapped_column(String(1500))
...
class Album(Base):
    __tablename__ = 'album'
    album_id: Mapped[int] = mapped_column(BigInteger, primary_key=True, autoincrement=False)
    album_handle: Mapped[Optional[str]] = mapped_column(String(150))
...
class Track(Base):
    __tablename__ = 'track'
    track_id: Mapped[int] = mapped_column(BigInteger, primary_key=True, autoincrement=False)
    ...
    track_composer: Mapped[Optional[str]] = mapped_column(String(255))
...
class StatsEchonest(Base):
    ...
    track_id: Mapped[int] = mapped_column(BigInteger, ForeignKey('sae.track.track_id', ondelete="CASCADE"), primary_key=True)
...
class User(Base):
    ...
    user_gender: Mapped[Optional[str]] = mapped_column(String(1))
...
class Playlist(Base):
    ...
    # Correction de la relation secondaire avec le nouveau type BigInteger
...
class ArtistAlbumTrack(Base):
    ...
    artist_id: Mapped[int] = mapped_column(BigInteger, ForeignKey('sae.artist.artist_id', ondelete="CASCADE"), primary_key=True)
    album_id: Mapped[int] = mapped_column(BigInteger, ForeignKey('sae.album.album_id', ondelete="CASCADE"), primary_key=True)
    track_id: Mapped[int] = mapped_column(BigInteger, ForeignKey('sae.track.track_id', ondelete="CASCADE"), primary_key=True)
